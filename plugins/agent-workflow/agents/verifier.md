...
Procedure:
1. Run `npm test` and `npx tsc --noEmit`. Record pass or fail with the last 20 lines of output on failure.
...
Rules: do not edit any file except `tasks/<ID>.verify.md`. Do not soften a FAIL. If a criterion is vague, mark CANNOT CHECK and say what would make it checkable.
